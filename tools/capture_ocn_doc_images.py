...
if TYPE_CHECKING:
    from mapir_camera_core.spectral_indices import MissingBandError, SpectralIndexParams
...
def filter_available_indices(indices: Iterable[str], bands: Dict[str, int], params) -> List[str]:
    from mapir_camera_core.spectral_indices import MissingBandError, compute_spectral_indices
...
def build_band_channels(filter_set: str, params: Dict[str, Any]) -> Dict[str, int]:
    from mapir_camera_core.spectral_indices import preset_band_channels
...
def open_camera(
    ...
    if use_gstreamer and pipeline:
        ...
        if not cap.isOpened():
            # Try a minimal gstreamer pipeline as a fallback.
            gst_pipeline = (
                f"v4l2src device={device} ! image/jpeg,width={width},height={height},framerate={int(fps)}/1 "
...
def save_index_image(
    ...
    from mapir_camera_core.colormap import colorize_scalar_field
...
def main() -> int:
    ...
    parser.add_argument("--index", action="append", default=[], help="Index name to render (repeatable).")
    parser.add_argument("--all", action="store_true", help="Render all OCN-available indices.")
    parser.add_argument(
        "--camera-config",
    ...
    camera_keys = ["video_device", "pixel_format", "use_gstreamer", "gstreamer_pipeline", "image_width", "image_height", "framerate"]
    indices_keys = [
        "indices",
        "filter_set",
        "normalize_input",
    ...
    from mapir_camera_core.spectral_indices import (
        SpectralIndexParams,
